  ...
  return (
    <div className="relative overflow-x-hidden">
      ...
      <main id="topo">
        ...
        <Section className="bg-[var(--ink)]">
          <Reveal>
            <p className="eyebrow">Comparativo qualitativo</p>
            <h2 className="mt-5 text-[clamp(1.9rem,5vw,3.4rem)] leading-[1]">O MERCADO.</h2>
            <p className="mt-6 max-w-2xl text-sm leading-[1.9] text-muted-foreground">
              Comparação qualitativa de referência. Valores dos concorrentes não definidos neste
              projeto aparecem como “a verificar”. A Element é uma marca fictícia de projeto
              acadêmico e não possui participação de mercado.
            </p>
          </Reveal>
          ...
          <Reveal delay={100}>
            <div className="mt-12 space-y-6 md:hidden">
              {concorrentes.map((c) => (
                <div
                  key={c.marca}
                  className={`border-t pt-5 ${c.own ? "border-primary" : "border-border"}`}
                >
                  <h3
                    className={`text-lg tracking-[0.16em] uppercase ${c.own ? "text-primary" : ""}`}
                  >
                    {c.marca}
                  </h3>
                  <dl className="mt-4 grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-2 text-sm">
                    {[
                      ["Volume", c.volume],
                      ["Posicionamento", c.pos],
                      ["Preço", c.preco],
                      ["Identidade", c.id],
                      ["Público", c.pub],
                    ].map(([k, v]) => (
                      <div key={k} className="contents">
                        <dt className="text-xs tracking-[0.18em] text-steel uppercase">{k}</dt>
              ...
              <table className="w-full border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-primary/50">
                    {["Marca", "Volume", "Posicionamento", "Preço", "Identidade", "Público"].map(
                      (h) => (
                        <th
                          key={h}
                          className="py-4 pr-6 text-xs font-semibold tracking-[0.2em] text-primary uppercase"
                        >
                          {h}
                        </th>
                      ),
                ...
                <tbody>
                  {concorrentes.map((c) => (
                    <tr key={c.marca} className="border-b border-border">
                      <td
                        className={`py-5 pr-6 tracking-[0.12em] uppercase ${
                          c.own ? "text-primary" : "text-foreground"
                        }`}
                      >
                        {c.marca}
                      </td>
                      <td className="py-5 pr-6 text-muted-foreground">{c.volume}</td>
                      <td className="py-5 pr-6 text-muted-foreground">{c.pos}</td>
                      <td className="py-5 pr-6 text-muted-foreground">{c.preco}</td>
                      <td className="py-5 pr-6 text-muted-foreground">{c.id}</td>
                      <td className="py-5 pr-6 text-muted-foreground">{c.pub}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
